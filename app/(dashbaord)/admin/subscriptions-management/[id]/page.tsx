...
export default function SubscriptionDetailsPage() {
  ...
  const [activeTab, setActiveTab] = useState<'details' | 'migration' | 'jobs' | 'garages'>('details');
  const [selectedJobType, setSelectedJobType] = useState<JobType | undefined>(undefined);

  const { data: subscription, isLoading: isLoadingSubscription } = useGetASubscriptionQuery(
    subscriptionId,
    { skip: !subscriptionId }
  );
  ...
  if (isLoadingSubscription) {
    return (
      <div className="flex justify-center items-center min-h-[60vh]">
        ...
        <span className="ml-3 text-gray-600 font-medium">Loading subscription details...</span>
      </div>
    );
  }
  ...
  if (!subscription) {
    return (
      <div className="text-center py-16">
        <div className="text-red-600 mb-4 font-semibold">Subscription not found</div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg border border-gray-200">
        ...
        <div className="p-6">
          {activeTab === 'details' && (
            <SubscriptionDetailsTab subscription={subscription} subscriptionId={subscriptionId} />
          )}
          {activeTab === 'migration' && (
